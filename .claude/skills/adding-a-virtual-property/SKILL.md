...
No registration is needed. The `LcmMetaDataCache` automatically discovers properties with `[VirtualProperty]` via reflection during initialization.

FLIDs are auto-assigned from 20,000,001 upward, in the order the attributes are constructed, with a ceiling of 30,000,000. Because the number depends on construction order, never persist a virtual FLID or hard-code one -- look it up by name through `MetaDataCache.GetFieldId`.
...
Partial interface extensions live in `src/SIL.LCModel/InterfaceAdditions.cs`, which already
carries about eighty of them. Add yours there rather than starting a new file.

```csharp
// In src/SIL.LCModel/InterfaceAdditions.cs
