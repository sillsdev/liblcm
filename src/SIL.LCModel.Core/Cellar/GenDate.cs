...
namespace SIL.LCModel.Core.Cellar
{
	...
	public struct GenDate : IEquatable<GenDate>, IComparable, IComparable<GenDate>
	{
		...
		public string ToLongString()
		{
			...
			switch (m_precision)
			{
				case PrecisionType.Approximate:
					sb.Append(Resources.ksGenDateApprox);
					sb.Append(" ");
					break;
				case PrecisionType.After:
					sb.Append(Resources.ksGenDateAfter);
					sb.Append(" ");
					break;
				case PrecisionType.Before:
					sb.Append(Resources.ksGenDateBefore);
					sb.Append(" ");
					break;
			}
			...
			{
				sb.Append(Resources.ksGenDateAD);
				sb.Append(" ");
			}
			...
			if (m_year != UnknownYear)
			{
				...
				{
					sb.Append(" ");
					sb.Append(Resources.ksGenDateBC);
				}
			}
			return sb.ToString();
		}
		...
		public static bool TryParse(string date, out GenDate gen)
		{
			...
			date = date.Trim();
			if (date.StartsWith(Resources.ksGenDateApprox))
			{
				precision = PrecisionType.Approximate;
				date = date.Substring(Resources.ksGenDateApprox.Length + 1);
			}
			else if (date.StartsWith(Resources.ksGenDateAfter))
			{
				precision = PrecisionType.After;
				date = date.Substring(Resources.ksGenDateAfter.Length + 1);
			}
			else if (date.StartsWith(Resources.ksGenDateBefore))
			{
				precision = PrecisionType.Before;
				date = date.Substring(Resources.ksGenDateBefore.Length + 1);
			}
			if (date.Contains(Resources.ksGenDateAD + " "))
			{
				fAD = true;
				date = date.Replace(Resources.ksGenDateAD + " ", "");
			}
			else if (date.EndsWith(Resources.ksGenDateBC))
			{
				fAD = false;
				date = date.Substring(0, date.Length - (Resources.ksGenDateBC.Length + 1));
			}
			int nDay = UnknownDay;
