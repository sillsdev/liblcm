...
namespace SIL.LCModel.Core.Phonology
{
	...
	public class PhonEnvRecognizer
	{
		...
		public static void CreateErrorMessageFromXml(string strRep1, string sXmlMessage, out int pos, out string sMessage)
		{
			...
			bldrMsg.AppendFormat(Resources.ksBadEnv, strRep);
			if (sStatus == "class")
			{
				int iRightBracket = strRep.Substring(pos).IndexOf(']');
				string sClass = strRep.Substring(pos, iRightBracket);
				bldrMsg.AppendFormat(Resources.ksBadClassInEnv, sClass);
			}
			if (sStatus == "segment")
			{
				string sPhoneme = strRep.Substring(pos);
				bldrMsg.AppendFormat(Resources.ksBadPhonemeInEnv, sPhoneme);
			}
			if (sStatus == "missingClosingParen")
			{
				bldrMsg.AppendFormat(Resources.ksMissingCloseParenInEnv, strRep.Substring(pos));
			}
			if (sStatus == "missingOpeningParen")
			{
				bldrMsg.AppendFormat(Resources.ksMissingOpenParenInEnv, strRep.Substring(pos));
			}
			if (sStatus == "missingClosingSquareBracket")
			{
				bldrMsg.AppendFormat(Resources.ksMissingCloseBracketInEnv, strRep.Substring(pos));
			}
			if (sStatus == "missingOpeningSquareBracket")
			{
				bldrMsg.AppendFormat(Resources.ksMissingOpenBracketInEnv, strRep.Substring(pos));
			}
			if (sStatus == "syntax")
			{
				bldrMsg.AppendFormat(Resources.ksBadEnvSyntax, strRep.Substring(pos));
			}
