...
namespace SIL.LCModel.Core.Text
{
	...
	public static class TsStringUtils
	{
		...
		/// as well as a flag indicating whether the change requires analysis adjustment.
		...
		/// <summary>
		/// Determines whether the change requires a forced analysis adjustment.
		/// We force analysis adjustment for run text and ktptObjData changes.
		///
		...
		/// If a user updates a Writing System code, adjusting analyses at the wrong time will invalidate the users work and duplicate all analyses.
		/// if a Writing System code is the only change, we must *not* adjust analyses (https://jira.sil.org/browse/LT-20344) during the change,
		/// because this will invalidate the user's analyses and create duplicates; someone else will notice any needed adjustments later.
		/// For all other property changes (most are style changes), it doesn't matter, and it's easier not to force analysis adjustments.
		/// </summary>
