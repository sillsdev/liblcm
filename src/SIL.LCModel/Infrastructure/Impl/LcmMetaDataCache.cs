...
namespace SIL.LCModel.Infrastructure.Impl
{
	...
	internal sealed class LcmMetaDataCache : IFwMetaDataCacheManaged, IFwMetaDataCacheManagedInternal
	{
		...
		/// </summary>
		/// <exception cref="LcmInvalidFieldException">
		...
		{
			if (!m_metaFieldRecords.ContainsKey(flid))
				throw new LcmInvalidFieldException("Invalid field identifier: " + flid, "flid");
		}

		private void CheckClid(int clid)
		{
			if (!m_metaClassRecords.ContainsKey(clid))
				throw new LcmInvalidClassException("Invalid class identifier: " + clid, "clid");
		}

		private void CheckClid(string className)
		{
			if (!m_nameToClid.ContainsKey(className))
				throw new LcmInvalidClassException("Invalid class name: " + className, className);
		}
