...
using CmSemanticDomain = SIL.LCModel.DomainImpl.CmSemanticDomain;
using LexEntry = SIL.LCModel.DomainImpl.LexEntry;
using LexSense = SIL.LCModel.DomainImpl.LexSense;
using PhEnvironment = SIL.LCModel.DomainImpl.PhEnvironment;
...
namespace SIL.LCModel.Infrastructure.Impl
{
	...
	internal partial class LexEntryRepository : ILexEntryRepositoryInternal
	{
		...
		/// </summary>
		public int HomographMorphOrder(LcmCache cache, IMoMorphType morphType)
		{
		...
		/// <returns></returns>
		public List<ILexEntry> FindEntriesForWordform(LcmCache cache, ITsString tssWf, IWfiAnalysis wfa, ref bool duplicates)
		{
		...
		public List<ILexEntry> FindEntriesForWordformWorker(LcmCache cache, ITsString tssWf, IWfiAnalysis wfa, ref bool duplicates)
		{
			if (tssWf == null)
