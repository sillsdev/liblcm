...
namespace SIL.LCModel.Infrastructure.Impl
{
	...
	class ChangeReconciler : IReconcileChanges
	{
		...
		private bool OkToReconcileDirtball(ICmObjectSurrogate dirtball)
		{
			...
			{
				if (action is LcmDateTimePropertyChanged)
				{
					var dtChange = (LcmDateTimePropertyChanged) action;
				...
				else if (action is LcmVectorPropertyChanged)
				{
					var flid = ((LcmVectorPropertyChanged) action).ModifiedFlid;
		...
		{
			var uow = new NonUndoableUnitOfWork(UowService);
					...
					action =>
						{
								...
								if (mdc.GetFieldName(flid) == "DateModified")
								{
									...
									foreach (var undoStack in UowService.UndoStacks)
									{
										foreach (var uow1 in undoStack.UnsavedUnitsOfWork)
										{
											...
											{
												var dateChange = change as LcmDateTimePropertyChanged;
							...
							{
								ReconcileCollectionProperty((LcmVectorPropertyChanged) action);
							}
			...
			foreach (var goner in m_foreignGoners)
			{
				...
				uow.AddAction(new LcmStateChangeObjectDeletion(existingObj, ((ICmObjectInternal)existingObj).ToXmlString()));
		...
		private void MakeBundlePredateUnsavedBundles(NonUndoableUnitOfWork uow)
		{
		...
		/// </summary>
		private void ReconcileCollectionProperty(LcmVectorPropertyChanged foreignChange)
		{
			...
			foreach (var undoStack in UowService.UndoStacks)
			{
				foreach (var uow1 in undoStack.UnsavedUnitsOfWork)
				{
					foreach (var change in uow1.Changes.ToArray())
					{
						var vecChange = change as LcmVectorPropertyChanged;
		...
		/// </summary>
		private void MakeChangeInfoFor(ICmObjectSurrogate dirtball, Func<LcmStateChangeBase, bool> processChangeRecord)
		{
			...
			if (foreignOwnerGuid != currentOwnerGuid)
				if (processChangeRecord(new LcmOwnerChanged(currentObj, currentOwnerGuid, currentObj.OwningFlid, foreignOwnerGuid, 0)))
					return;
				...
				switch (propertyType)
				{
					...
					case CellarPropertyType.ReferenceCollection:
						{
							...
							if (currentSet.SetEquals(foreignSet))
								continue;
							if (processChangeRecord(new LcmVectorPropertyChanged(currentObj, flid, currentItems, foreignItems)))
								return;
						}
						break;
					...
					case CellarPropertyType.ReferenceAtomic:
						{
							...
							if (currentItem != foreignItem)
							{
								if (propertyType == CellarPropertyType.OwningAtomic)
									processChangeRecord(new LcmGuidPropertyChanged(currentObj, flid, currentItem, foreignItem));
								else
									if (processChangeRecord(new LcmAtomicRefPropertyChanged(currentObj, flid, currentItem, foreignItem)))
										return;
							}
						}
						break;
					...
					case CellarPropertyType.Time:
						...
						if (IsTimeRoughlyEqual(originalTime, newTime))
							continue;
						if (processChangeRecord(new LcmDateTimePropertyChanged(currentObj, flid, originalTime, newTime)))
							return;
						break;
					...
					case CellarPropertyType.String:
						{
							...
							if ((current == null && !IsNullOrEmptyTss(foreign)) || (current != null && !current.Equals(foreign)))
								if (processChangeRecord(new LcmTsStringPropertyChanged(currentObj, flid, current, foreign)))
									return;
						}
						break;
					case CellarPropertyType.Boolean:
						{
							...
							if (current != foreign)
								if (processChangeRecord(new LcmBooleanPropertyChanged(currentObj, flid, current, foreign)))
									return;
						}
						break;
					case CellarPropertyType.GenDate:
						{
							...
							if (current != foreign)
								if (processChangeRecord(new LcmGenDatePropertyChanged(currentObj, flid, current, foreign)))
									return;
						}
						break;
					case CellarPropertyType.Guid:
						{
							...
							if (current != foreign)
								if(processChangeRecord(new LcmGuidPropertyChanged(currentObj, flid, current, foreign)))
									return;
						}
						break;
					case CellarPropertyType.Integer:
						{
							...
							if (current != foreign)
								if (processChangeRecord(new LcmIntegerPropertyChanged(currentObj, flid, current, foreign)))
									return;
						}
						break;
					case CellarPropertyType.Binary:
						{
							...
							if (sig == "ITsTextProps")
							{
								...
								if (current != foreign)
									if (processChangeRecord(new LcmTsTextPropsPropertyChanged(currentObj, flid, current, foreign)))
										return;
							}
							else
							{
								...
								if (!EqualByteArrays(foreign, current))
									if (processChangeRecord(new LcmBinaryPropertyChanged(currentObj, flid, current, foreign)))
										return;
							}
						}
						break;
					case CellarPropertyType.Unicode:
						{
							...
							if (string.IsNullOrEmpty(foreign) && string.IsNullOrEmpty(current))
								continue;
							if (current != foreign)
								if (processChangeRecord(new LcmStringPropertyChanged(currentObj, flid, current, foreign)))
									return;
						}
						break;
				}
