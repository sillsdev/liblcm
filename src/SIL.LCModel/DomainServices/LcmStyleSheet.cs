...
//
// FwStylesheet uses LCM cache, and its database, to maintain a collection of StStyles
...
namespace SIL.LCModel.DomainServices
{
	...
	/// The StStyle objects, defined by the LCM, maintain the style information in
	/// object/property form in an LCM cache.
	...
	///	</remarks>
	/// ----------------------------------------------------------------------------------------
	public class LcmStyleSheet : IVwStylesheet, IStylesheet
	{
		...
		private LcmStyleSheet m_styleSheetWithUiStyle;
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		private LcmStyleSheet StyleSheetWithUiStyle
		{
			get
			{
				if (m_styleSheetWithUiStyle == null)
				{
					...
					{
						m_styleSheetWithUiStyle = new LcmStyleSheet();
		...
		///   --------------------------------------------------------------------------------------------
		///   <summary>
		///  	FwStyleSheet.Init() sets the LcmCache, the hvo of the owning object, and the tag
		...
		/// --------------------------------------------------------------------------------------------
		public void Init(LcmCache cache, int hvoStylesOwner, int tagStylesList)
		{
			m_cache = cache;
		...
		protected void LoadStyles()
		{
			...
			// Use LcmCache to create an array of style objects, one for each style
			// in the collection owned by m_hvoStylesOwner.
			foreach (var hvo in ((ISilDataAccessManaged)m_cache.DomainDataByFlid).VecProp(m_hvoStylesOwner, m_tagStylesList))
			{
		...
		/// -----------------------------------------------------------------------------------
		/// <summary>
		/// Store a style. In particular, store the details in the LCM cache and the derived
		...
		{
			IStStyle lcmStyle = null; // our local reference
			...
			if (lcmStyle == null)
				lcmStyle = m_cache.ServiceLocator.GetInstance<IStStyleRepository>().GetObject(hvoStyle);

			// Save the style properties in the LcmCache's style object
			lcmStyle.Name = sName;
			lcmStyle.Usage.UserDefaultWritingSystem = TsStringUtils.MakeString(sUsage, m_cache.WritingSystemFactory.UserWs);
			if (hvoBasedOn > 0)
				lcmStyle.BasedOnRA = m_cache.ServiceLocator.GetInstance<IStStyleRepository>().GetObject(hvoBasedOn);
			if (hvoNext > 0)
				lcmStyle.NextRA = m_cache.ServiceLocator.GetInstance<IStStyleRepository>().GetObject(hvoNext);
			lcmStyle.Type = (StyleType)nType;
			lcmStyle.IsBuiltIn = fBuiltIn;
			lcmStyle.IsModified = fModified;
			lcmStyle.Rules = ttp;
			...
			{
				m_StyleInfos.Add(new BaseStyleInfo(lcmStyle));
		...
		{
			get { return m_cache.DomainDataByFlid; }
		}
		...
		/// -----------------------------------------------------------------------------------
		/// <summary>
		/// Return an HVO for a newly created style, using the LCM cache.
		/// Create a new StStyle object in the LCM cache, and return its new hvo.
		/// Insert the new style into its owner property (e.g. the StylesList property
		///  of LangProject or Scripture).
		/// This is a default implementation using the LCM cache. Specialized style sheets
		...
		{
			return m_cache.DomainDataByFlid.MakeNewObject(StStyleTags.kClassId, m_hvoStylesOwner, m_tagStylesList, -1);
		}
			...
			foreach (BaseStyleInfo styleInfo in m_StyleInfos)
			{
				...
				if (style.Hvo == hvoStyle)
				{
					...
					else
					{
						...
						SetupUndoStylesheet(true); // to perform ComputeDerivedStyles when undoing
						m_cache.DomainDataByFlid.DeleteObj(hvoStyle);
		...
		{
			CopyStyleContextValues(m_cache.ServiceLocator.GetInstance<IStStyleRepository>().GetObject(hvoStyle),
				m_cache.ServiceLocator.GetInstance<IStStyleRepository>().GetObject(hvoBasedOn));
		}
...
//		}
	...
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	public class InMemoryStyleSheet : LcmStyleSheet
	{
