...
namespace SIL.LCModel.DomainServices.DataMigration
{
	...
	/// Otherwise we make one the best we can from the old writing system LCM object.
	///
	...
	internal class DataMigration7000019 : IDataMigration
	{
			...
			if (!string.IsNullOrEmpty(domainObjectDtoRepository.ProjectFolder))
			{
				foreach (Tuple<string, DomainObjectDTO, XElement> wsInfo in guidToWsInfo.Values)
				{
					if (referencedWsIds.Contains(wsInfo.Item1))
					{
						...
						string globalPath = Path.Combine(LcmFileHelper.OldGlobalWritingSystemStoreDirectory, ldmlFileName);
						...
						FillWritingSystemFromLcm(domainObjectDtoRepository, wsInfo.Item3, ws);
						ws.Save(localPath);
					}
				}
			}
		...
		private static void FillWritingSystemFromLcm(IDomainObjectDTORepository domainObjectDtoRepository,
					...
					if (!string.IsNullOrEmpty(variant))
					{
						...
						if (variantSubtags.Any(x => x.IsPrivateUse && x.Code == "etic"))
						{
							...
							string normalVariants =
								string.Join("-", newVariantList.Select(v => v.IsPrivateUse ? "" : v.Code));
							if (normalVariants.Length > 1)
							{
								normalVariants = $"-{normalVariants}";
							}
							string privateVariants = string.Join("-",
								newVariantList.Where(v=>v.Code != "fonipa").Select(v=>v.IsPrivateUse ? v.Code : ""));
