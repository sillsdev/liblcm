...
namespace SIL.LCModel.DomainServices.DataMigration
{
	...
	internal class DataMigration7000072 : IDataMigration
	{
		...
		/// </summary>
		/// <param name="repoDto"></param>
		public void AddSensesToReversalIndexEntry_CorrectlyWorking(IDomainObjectDTORepository repoDto)
		{
			...
			foreach (var aLexSense in allLexSenses)
			{
				...
				foreach (var aReversalEntry in reversalEntriesList)
				{
					...
					var allReversalIndexEntries = repoDto.AllInstancesWithSubclasses("ReversalIndexEntry");
					...
					foreach (var aReversalIndexEntry in allReversalIndexEntries)
					{
						...
						if (reversalIndexEntryGuid == reversalEntryGuid)
						{
		...
		public void AddSensesToReversalIndexEntry(IDomainObjectDTORepository repoDto)
		{
			...
			foreach (var aLexSense in allLexSenses)
			{
				...
				foreach (var aReversalEntry in reversalEntriesList)
				{
					// Get the Current ReversalEntries (objsur) Guid
					var reversalEntryGuid = aReversalEntry.Attribute("guid")?.Value;

					// Get the Reversal Entry Object that has the specified Guid
