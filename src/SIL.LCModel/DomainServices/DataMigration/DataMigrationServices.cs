...
namespace SIL.LCModel.DomainServices.DataMigration
{
	...
	internal static class DataMigrationServices
	{
			...
			Tuple<string, string, string>[] languageAbbrAndNames, DateTime createTime, int wsSelector, string[] possibilityItems = null, int itemType = 7)
		{
			...
			sb.Append("</Abbreviation>");
			sb.AppendFormat("<DateCreated val=\"{0}\" />", createTime.ToLCMTimeFormatWithMillisString());
			sb.AppendFormat("<DateModified val=\"{0}\" />", createTime.ToLCMTimeFormatWithMillisString());
			...
			string abbr, DateTime createTime, string className = "CmPossibility")
		{
			...
			sb.Append("</Abbreviation>");
			sb.Append($"<DateCreated val=\"{createTime.ToLCMTimeFormatWithMillisString()}\" />");
			sb.Append($"<DateModified val=\"{createTime.ToLCMTimeFormatWithMillisString()}\" />");
