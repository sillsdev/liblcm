...
namespace SIL.LCModel.Application.ApplicationServices
{
	...
	public class XmlImportData
	{
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public XmlImportData(LcmCache cache, bool fCreateMissingLinks)
		{
			m_cache = cache;
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Import the file contents into the database represented by the LcmCache established
		/// by the constructor.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		...
		/// ------------------------------------------------------------------------------------
		/// <summary>
		/// Import the text reader contents into the database represented by the LcmCache set
		/// in the constructor.
		/// </summary>
		/// ------------------------------------------------------------------------------------
		...
		private bool AreEntryRefTypesCompatible(ILcmReferenceSequence<ILexEntryType> oldTypes,
			ILcmReferenceSequence<ILexEntryType> newTypes)
		{
		...
		private bool AreEntryRefTargetsCompatible(ILcmReferenceSequence<ICmObject> oldTargets,
			ILcmReferenceSequence<ICmObject> newTargets)
		{
		...
		private void ReadGenDateValue(XmlReader xrdr, FieldInfo fi)
		{
			...
			GenDate gdt = CmObject.GetGenDateFromInt(Int32.Parse(sVal));
		...
		private ILexSense GetIndicatedSense(string sSenseNumber, ILexEntry le)
		{
			...
			ILcmOwningSequence<ILexSense> rgls = le.SensesOS;
			ILexSense ls = rgls[0];
		...
		private ICmFolder GetDesiredCmFolder(string sFolderName, ILcmOwningCollection<ICmFolder> rgfolders)
		{
		...
		private void AddFormsToFormEntryMap(ILcmOwningCollection<IReversalIndexEntry> entries)
		{
		...
		private void AddFormsToFormEntryMap(ILcmOwningSequence<IReversalIndexEntry> entries)
		{
			foreach (IReversalIndexEntry rie in entries)
			{
				AddFormToFormEntryMap(rie);
		...
		private Dictionary<WsString, int> CreateListNameDict(int flid)
		{
			...
			if (cpl != null)
			{
				ILcmOwningSequence<ICmPossibility> seqPoss = cpl.PossibilitiesOS;
				if (seqPoss != null)
		...
		private void InitializeByNames(Dictionary<WsString, int> listNameDict,
			ILcmOwningSequence<ICmPossibility> seqPoss)
		{
