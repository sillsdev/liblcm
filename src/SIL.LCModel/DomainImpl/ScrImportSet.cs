...
namespace SIL.LCModel.DomainImpl
{
	...
	internal partial class ScrImportSet
	{
			...
			set
			{
				lock (SyncRoot)
				{
					...
					if (value != null && (value == ParatextNotesProj || value == ParatextBTProj))
						throw new ArgumentException(ScrResources.kstidPtScrAlreadyUsed);
			...
			set
			{
				lock (SyncRoot)
				{
					...
					if (value != null && (value == ParatextScrProj || value == ParatextNotesProj))
						throw new ArgumentException(ScrResources.kstidPtBtAlreadyUsed);
			...
			set
			{
				lock (SyncRoot)
				{
					...
					if (value != null && (value == ParatextScrProj || value == ParatextBTProj))
						throw new ArgumentException(ScrResources.kstidPtNotesAlreadyUsed);
		...
		/// ------------------------------------------------------------------------------------
		private void SaveMappings(ILcmOwningCollection<IScrMarkerMapping> mappingsOC, ScrMappingList mappingInfoList)
		{
