...
namespace SIL.LCModel.DomainImpl
{
	...
	internal partial class StTxtPara
	{
		...
		/// ------------------------------------------------------------------------------------
		protected virtual IStFootnote CreateBlankDummyFootnote(ILcmOwningSequence<IStFootnote> sequence,
		...
		/// ------------------------------------------------------------------------------------
		protected virtual IStFootnote CreateBlankDummyFootnoteNoRecursion(ILcmOwningSequence<IStFootnote> sequence,
			int iFootnote, ITsString paraContents, int iRun)
		{
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public virtual ILcmOwningSequence<IStFootnote> FootnoteSequence
		{
		...
		public void SetCloneProperties(ICmObject clone)
		{
			...
			CopyObject<ISegment>.CloneLcmObjects(SegmentsOS, x => clonedPara.SegmentsOS.Add(x));
			CopyObject<ICmTranslation>.CloneLcmObjects(TranslationsOC, x => clonedPara.TranslationsOC.Add(x));
			CopyObject<ICmObject>.CloneLcmObjects(AnalyzedTextObjectsOS, x => clonedPara.AnalyzedTextObjectsOS.Add(x));
			...
			out int isegMinLow, out int isegMinHigh, out int isegLim)
		{
			...
			for (int i = 0; i < para.SegmentsOS.Count; i++)
			{
				...
				// verse as one segment group. However, moving the code to LCM complicates that
		...
		protected override void ValidateAddObjectInternal(AddObjectEventArgs e)
		{
			...
			if (e.Flid == StTxtParaTags.kflidTranslations)
			{
				...
				var trans = (ICmTranslation) e.ObjectAdded;
