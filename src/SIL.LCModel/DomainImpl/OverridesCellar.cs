...
// make LCM independent of SWF and so we can't reference PhraseTranslationHelper which defines
...
namespace SIL.LCModel.DomainImpl
{
	...
	internal partial class StPara
	{
		...
		public bool IsFinalParaInText
		{
			get
			{
				ILcmOwningSequence<IStPara> paras = ((IStText)Owner).ParagraphsOS;
	...
	internal partial class CmSemanticDomain
	{
		...
		public IEnumerable<ILexSense> ReferringSenses
		{
			get
			{
				...
				{
					var collection = item as LcmReferenceCollection<ICmSemanticDomain>;
					if (collection == null)
						continue;
	...
	internal partial class CmPossibilityList
	{
		...
		public ICmPossibility FindOrCreatePossibility(string possibilityPath, int ws, bool fFullPath)
		{
			...
			if (m_possibilityMap.TryGetValue(ws, out wsHashTable))
			{
				...
				ILcmOwningSequence<ICmPossibility> possibilityList = PossibilitiesOS;
				int level = 1;
		...
		/// </summary>
		/// ------------------------------------------------------------------------------------
		public ICmPossibility FindPossibilityByName(ILcmOwningSequence<ICmPossibility> possList, string possibilityPath, int ws)
		{
		...
		/// </summary>
		/// -----------------------------------------------------------------------------------
		private void CacheNoteCategories(ILcmOwningSequence<ICmPossibility> possibilityList, int ws)
		{
			if (!m_possibilityMap.ContainsKey(ws))
	...
	internal partial class CmPossibility
	{
		...
		/// <returns></returns>
		internal static ITsString BestAnalysisName(LcmCache cache, ICmPossibility pss)
		{
		...
		/// <param name="pss"></param>
		/// <returns></returns>
		internal static ITsString BestAnalysisOrVernName(LcmCache cache, ICmPossibility pss)
		{
		...
		/// <returns></returns>
		internal static ITsString BestAnalysisOrVernName(LcmCache cache, ICmPossibility pss, string defValue)
		{
			return BestAlternative(cache, pss,
		...
		protected static ITsString BestAlternative(LcmCache cache, ICmPossibility pss, int wsMagic, int flid, string defValue)
		{
			ITsString tss = null;
			if (pss != null)
		...
		/// <returns></returns>
		internal static ITsString BestAnalysisOrVernAbbr(LcmCache cache, int hvo)
		{
			ITsString tss = null;
			if (hvo != 0)
			{
		...
		/// <param name="pss"></param>
		/// <returns></returns>
		internal static ITsString BestVernOrAnalysisAbbreviation(LcmCache cache, ICmPossibility pss)
		{
			return BestAlternative(cache, pss,
		...
		/// <param name="ws"></param>
		/// <returns></returns>
		internal static ITsString TSSAbbrforWS(LcmCache cache, ICmPossibility pss, int ws)
		{
			ITsString tss = null;
			if (pss != null)
			{
		...
		public bool IsDefaultDiscourseTemplate
		{
			get
			{
				...
				ILcmOwningSequence<ICmPossibility> discourseTemplates = ccTempl.PossibilitiesOS;
		...
		public bool IsOnlyTextMarkupTag
		{
			get
			{
				...
				ILcmOwningSequence<ICmPossibility> tagTypes = tags.PossibilitiesOS;
	...
	internal partial class FsFeatureSystem
	{
		...
		internal static void SetInnerText(LcmCache cache, ITsMultiString item, XmlNode node)
		{
			if (node != null)
			{
	...
	internal partial class FsOpenValue
	{
		/// <summary>
		/// True if the two are 'equivalent'.
		/// Since OpenValue isn't used yet, this is something of a skeleton. We just check
	...
	internal partial class FsFeatStruc
	{
		...
		public void PriorityUnion(IFsFeatStruc fsNew)
		{
			...
			{
				CopyObject<IFsFeatureSpecification>.CloneLcmObject(spec,
		...
		public override ITsString ChooserNameTS
		{
			get
			{
				...
				if (result != null && result.Length > 0)
					return result;
				return m_cache.MakeUserTss(Strings.ksEmptyFeatureStructure);
			}
		}
