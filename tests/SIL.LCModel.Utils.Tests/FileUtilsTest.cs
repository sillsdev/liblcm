...
namespace SIL.LCModel.Utils
{
	...
	public class FileUtilsTest // can't derive from BaseTest because of dependencies
	{
		...
		/// <summary>
		/// Tests FileUtils.AreFilesIdentical() with different files of different sizes.
		/// </summary>
		[Test]
		...
		/// <summary>
		/// Tests FileUtils.AreFilesIdentical() with different files of the same size.
		/// </summary>
		[Test]
		...
		/// <summary>
		/// Tests FileUtils.AreFilesIdentical() with identical files.
		/// </summary>
		[Test]
		...
		/// <summary>
		/// Tests FileUtils.AreFilesIdentical() with identical files that have different creation times.
		/// </summary>
		[Test]
		...
		public void EnsureDirectoryExists_CreatesNonExistentDirectoryAndSuperDirectories()
		{
			...
			for (var superDir = Path.GetDirectoryName(directory); superDir != null; superDir = Path.GetDirectoryName(superDir))
			{
		...
		public void ApplyCaseByBinary()
		{
			...
			string actual = FileUtils.ApplyCaseByBinary(inputString, inputNumber);
		...
		public void ProduceCaseInsenstiveCombinations_one()
		{
			string input = "a";
			string expected = "a; A";
			string actual = FileUtils.ProduceCaseInsenstiveCombinations(input);
			Assert.AreEqual(expected, actual);
		}
		...
		public void ProduceCaseInsenstiveCombinationsFromMultipleTokens_1()
		{
			...
			string actual = FileUtils.ProduceCaseInsenstiveCombinationsFromMultipleTokens(input);
			Assert.AreEqual(expected, actual);
		}
		...
		private void FileDialogFilterCaseInsensitiveCombinations_TestHelper(List<FilterMap> filters)
		{
			...
			string input = inputBldr.ToString().TrimEnd('|');
			string expectedOutput = expectedOutputBldr.ToString().TrimEnd('|');
