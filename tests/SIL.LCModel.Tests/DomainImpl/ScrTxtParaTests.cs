...
namespace SIL.LCModel.DomainImpl
{
	...
	/// ----------------------------------------------------------------------------------------
	/// <summary>
	/// Test the <see cref="T:SIL.LCModel.DomainImpl.ScrTxtPara"/> class
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class ScrTxtParaTests : ScrInMemoryLcmTestBase
	{
		#region Data members
		...
		{
			AddDataToMatthew();
			ILcmOwningSequence<IScrSection> sections = m_book.SectionsOS;
		...
		public void FootnoteIndexTest_AfterDeletedStuff()
		{
			...
			ILcmOwningSequence<IStPara> paras = lastSection.ContentOA.ParagraphsOS;
			paras.RemoveAt(0);
	...
	/// <see cref="T:SIL.LCModel.DomainImpl.ScrTxtPara"/> class
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class GetRefsAtPositionTests : ScrInMemoryLcmTestBase
	{
		#region Data members
		private IScrBook m_book;
	...
	/// <summary>
	/// Test the GetRefsAtPosition method of the
	/// <see cref="T:SIL.LCModel.DomainImpl.ScrTxtPara"/> class when the paragraph contains
	/// a chapter number
	/// </summary>
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class GetRefsAtPositionTestsWithChapterNumbers : ScrInMemoryLcmTestBase
	{
		#region Data members
		private IScrBook m_book;
		private IScrSection m_section;
	...
	/// ----------------------------------------------------------------------------------------
	[TestFixture]
	public class ScrTxtParaTests_FootnoteRelated : ScrInMemoryLcmTestBase
	{
		#region Member data
		private IStText m_currentText;
		private IStText m_archivedText;
		private ILcmOwningSequence<IScrFootnote> m_archivedFootnotesOS;
		private ILcmOwningSequence<IScrFootnote> m_currentFootnotesOS;
		private IScrBook m_book;
		...
		public void CreateOwnedObjects_Footnote()
		{
			...
			IScrTxtPara para = CopyObject<IScrTxtPara>.CloneLcmObject((IScrTxtPara)m_archivedText[0],
		...
		{
			IScrTxtPara para = (IScrTxtPara)m_currentText[0];
			ILcmOwningSequence<IScrFootnote> footnotesOS = m_book.FootnotesOS;
		...
		public void CopyObject()
		{
			...
			// Now, call the method under test!
			IStTxtPara newPara = CopyObject<IStTxtPara>.CloneLcmObject(paraRev,
				x => m_currentText.ParagraphsOS.Add(x));
		...
		private void VerifyCopiedPara(IStTxtPara newPara)
		{
			...
			foreach (int ws in wsBt)
			{
				LcmTestHelper.VerifyBtFootnote(footnote1, newPara, ws, 2);
				...
				LcmTestHelper.VerifyBtFootnote(footnote2, newPara, ws, 6);
