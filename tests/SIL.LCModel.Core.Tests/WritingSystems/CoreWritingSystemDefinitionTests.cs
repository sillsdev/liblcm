	...
	public class CoreWritingSystemDefinitionTests : CloneableTests<CoreWritingSystemDefinition, WritingSystemDefinition>
	{
		...
		public void CopyCopiesAllNeededMembers()
		{
			...
			foreach (var fieldInfo in fieldInfos)
			{
				...
				// strings and other immutable types have special handling, so we won't worry about checking them here.
				if (!ImmutableTypes.Contains(fieldType))
				{
		...
		public HashSet<Type> ImmutableTypes { get; protected set; } = new HashSet<Type>
			{typeof(string)/*, typeof(CharacterSetDefinition)*/};
